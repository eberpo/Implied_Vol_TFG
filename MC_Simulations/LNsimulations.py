...
def simulate_price_parallel(i, j, T, K, F, sigma0, volup, volup_params, rhoin):
    tau = T[i]
    k = K[j]
    price = CondMC(F, k, sigma0, 1200, 1600, tau, volupdate=volup, volupdateparams=volup_params, rho=rhoin)
    return (j, i, price)

simparams=[[{'kappa': 3, 'theta': 0.46, 'vega': 0.3}, 0.3],  [{'kappa': 3, 'theta': 0.46, 'vega': 0.3}, 0.0],  
...
for simnum, params in enumerate(simparams):
    ...
    print('LN--->Starting Parallel Tasks; num of tasks =', incrementsT*incrementsK)
    results = Parallel(n_jobs=8)(delayed(simulate_price_parallel)(i, j, Tijms, Strikes, F, 0.43, Lognormal_Volatility, params[0], params[1]) for (i, j) in tasks)
    PricesSIM = np.zeros((incrementsK, incrementsT))
    for j, i, price in results:
        PricesSIM[j][i] = price
    PricesSIM = pd.DataFrame(PricesSIM)
    naam= str('prixSimsResults/LNCSimSurfaceResults' + str(simnum) + '.csv')
    print(naam)
